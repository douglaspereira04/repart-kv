...
// Helper function to perform a scan using lower_bound and iterator
// incrementation
template <typename StorageType, typename ValueType>
std::vector<std::pair<std::string, ValueType>>
scan_storage(StorageType &storage, const std::string &start_key, size_t limit) {
...
void test_scan_partial_prefix() {
    ...
    auto results =
        scan_storage<StorageType, ValueType>(storage, "user:1002", 10);
...
void test_scan_after_updates() {
    ...
    auto results1 =
        scan_storage<StorageType, ValueType>(storage, "prefix:", 10);
...
int main() {
    ...
    if (tests_failed == 0) {
        std::cout << "✓ All tests passed for all key storage implementations!"
                  << std::endl;
        return 0;
    } else {
