...
/**
 * @brief A weighted directed graph implementation using adjacency list representation.
 * 
 * This class provides efficient operations for managing vertices and edges with integer weights.
 * Both vertices and edges are identified by strings and have associated integer weights.
 */
class Graph {
private:
...
public:
    ...
    int increment_vertex_weight(const std::string& vertex) {
        // Using operator[] which creates entry with default value (0) if not exists
     ...
     */
    int increment_edge_weight(const std::string& source, const std::string& destination) {
        // Using operator[] on nested maps - creates entries with default value (0) if not exists
        // Then increment and return
     ...
     */
    int get_edge_weight(const std::string& source, const std::string& destination) const {
     ...
     */
    bool has_edge(const std::string& source, const std::string& destination) const {
        auto src_it = edges_.find(source);
        if (src_it != edges_.end()) {
