...
struct Operation {
    ...
    Operation(OperationType t, const std::string& k, const std::string& v = DEFAULT_VALUE_1KB, size_t l = 0)
...
inline std::vector<Operation> read_workload(const std::string& filepath) {
    ...
    while (std::getline(file, line)) {
        ...
        if (!std::getline(iss, key, ',')) {
            std::cerr << "Warning: Skipping malformed line " << line_number << " (missing key): " << line << std::endl;
            continue;
        }
        ...
        switch (command) {
            ...
            case 1: // WRITE
                ...
                operations.emplace_back(OperationType::WRITE, key, DEFAULT_VALUE_1KB);
                break;
            ...
            default:
                std::cerr << "Warning: Skipping line " << line_number 
                          << " (unknown command " << command << "): " << line << std::endl;
                break;
        }
    }
