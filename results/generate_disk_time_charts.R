#!/usr/bin/env Rscript
#
# Generate disk-usage-over-time charts with range bands from aggregated CSV files.
# Expects disk_kb_mean / disk_kb_min / disk_kb_max (KiB) from aggregate_throughput_time.py.
...
for (csv_file in csv_files) {
  ...
  if (!all(c("disk_kb_mean", "disk_kb_min", "disk_kb_max") %in% names(data))) {
    warning(paste("Skipping (no disk columns):", csv_file))
    next
  }
  ...
  data$disk_mib_mean <- data$disk_kb_mean / 1024
  data$disk_mib_min <- data$disk_kb_min / 1024
  data$disk_mib_max <- data$disk_kb_max / 1024
  ...
  p <- ggplot(data, aes(x = elapsed_s, y = disk_mib_mean, color = label, fill = label)) +
    geom_ribbon(aes(ymin = disk_mib_min, ymax = disk_mib_max), alpha = 0.2, color = NA) +
    geom_line(linewidth = 1, alpha = 0.6) +
    geom_point(data = repartitioning_starts,
               aes(x = elapsed_s, y = disk_mib_mean, color = label),
  ...
  output_file <- file.path(output_path, paste(workload, storage_engine, num_workers, thinking_time, sync_mode, "disk_time.png", sep = "."))
