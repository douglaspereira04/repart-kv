...
int main() {
    ...
    // ========================================
    std::cout << "\n--- Example 1: TkrzwHashKeyStorage (integers) ---" << std::endl;
    {
    ...
    // ========================================
    std::cout << "\n--- Example 2: TkrzwTreeKeyStorage (integers, sorted) ---" << std::endl;
    {
        ...
        while (!it2.is_end()) {
            std::cout << "  " << it2.get_key() << " = " << it2.get_value() << std::endl;
            ++it2;
        }
    }
    ...
    // ========================================
    std::cout << "\n--- Example 3: TkrzwTreeKeyStorage (long integers) ---" << std::endl;
    {
        ...
        while (!it.is_end()) {
            ...
            if (key.find("timestamp:") == 0) {
                std::cout << "  " << key << " = " << it.get_value() << std::endl;
            } else {
    ...
    // ========================================
    std::cout << "\n--- Example 4: TkrzwHashKeyStorage (pointers) ---" << std::endl;
    {
        TkrzwHashKeyStorage<std::string*> ptrStorage;
        
        if (!ptrStorage.is_open()) {
            std::cerr << "Failed to open hash key storage for pointers!" << std::endl;
            return 1;
        }
        ...
        if (ptrStorage.get("greeting", retrieved)) {
            ...
            std::cout << "Original pointer: " << str1 << ", Retrieved: " << retrieved << std::endl;
            std::cout << "Pointers match: " << (str1 == retrieved ? "Yes" : "No") << std::endl;
        }
    ...
    std::cout << "\n--- Example 5: Performance Comparison ---" << std::endl;
    {
        ...
        while (!tree_it.is_end() && tree_count < 5) {
            std::cout << "  " << tree_it.get_key() << " = " << tree_it.get_value() << std::endl;
            ++tree_it;
            tree_count++;
        }
    }
    ...
    std::cout << "  - TkrzwHashKeyStorage: Fast random access, unordered iteration" << std::endl;
    std::cout << "  - TkrzwTreeKeyStorage: Sorted keys, efficient range queries" << std::endl;
