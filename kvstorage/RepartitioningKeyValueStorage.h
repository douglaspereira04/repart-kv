...
/**
 * @brief CRTP abstract base class for repartitioning key-value storage
 * implementations
 *
 * This abstract class defines the common interface for repartitioning key-value
 * storage implementations using CRTP (Curiously Recurring Template Pattern) to
...
public:
    ...
    // These methods are implemented by derived classes as part of the CRTP
    // pattern Derived classes must implement:
    // - std::string read_impl(const std::string& key)
    // - void write_impl(const std::string& key, const std::string& value)
    // - std::vector<std::pair<std::string, std::string>> scan_impl(const
    // std::string& initial_key_prefix, size_t limit)
     ...
     */
    void repartition() { static_cast<Derived *>(this)->repartition_impl(); }
