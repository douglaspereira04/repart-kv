#!/usr/bin/env Rscript
#
# Generate disk-usage vs workers charts from aggregated throughput-time CSV files.
# For each engine, shows mean disk usage (MiB) per worker count with ±1 standard deviation
# (computed over the experiment time series).
#
# Usage:
#   Rscript generate_disk_workers_charts.R [input_path] [output_path] [max_y] [max_x] [width] [height] [show_titles]
...
chart_opts <- parse_chart_cli_args(
  ...
  default_output = "./charts/disk_workers"
)
...
jobs <- summarize_resource_by_workers_jobs(input_path, "disk_kb_mean")
if (length(jobs) == 0) {
  warning(paste("Warning: No disk data found in '", input_path, "'", sep = ""))
  quit(status = 0)
}

cat(paste("Found", length(jobs), "disk-by-workers chart job(s)\n"))
...
for (job in jobs) {
  ...
  cat(paste("Generating disk-by-workers chart for", workload, "-", storage_engine,
  ...
  p <- ggplot(subset_data, aes(
    ...
    labs(
      x = "Number of Workers",
      y = "Disk Usage (MiB)",
      title = paste("Disk Usage vs Workers:", workload, "-", storage_engine),
  ...
  output_file <- file.path(
    output_path,
    paste(c(safe_workload, safe_engine, thinking_time, sync_mode, p_parts, "disk_workers.png"), collapse = ".")
  )
...
cat(paste("\nAll disk-by-workers charts saved to:", output_path, "\n"))
