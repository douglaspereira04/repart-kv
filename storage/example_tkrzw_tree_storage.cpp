...
int main() {
    ...
    // ========================================
    std::cout << "\n--- Example 1: In-Memory Database (Sorted Keys) ---"
              << std::endl;
    {
    ...
    {
        std::cout << "Creating persistent database at: " << db_path
                  << std::endl;
              ...
              << std::endl;
    {
        ...
        std::cout << "Records before operations: " << engine.count()
                  << std::endl;
        ...
        std::string value;
        engine.read("config:max_connections", value);
        std::cout << "Updated config:max_connections = " << value << std::endl;
    }
    ...
    std::cout << "\n--- Example 4: Manual Locking Example ---" << std::endl;
    {
        ...
        auto writer = [&engine](int id) {
            for (int i = 0; i < 5; ++i) {
                std::string key = "thread:" + std::to_string(id) +
                                  ":count:" + std::to_string(i);
                std::string value = "thread_" + std::to_string(id) + "_value_" +
                                    std::to_string(i);
    ...
    // ========================================
    std::cout << "\n--- Example 5: Performance Test (with sorted scan) ---"
              << std::endl;
    {
        TkrzwTreeStorageEngine engine;
        ...
        std::cout << "Write throughput: "
                  << (num_entries * 1000 / write_duration.count()) << " ops/sec"
                  << std::endl;
        ...
        std::cout << "Read throughput: "
                  << (num_entries * 1000 / read_duration.count()) << " ops/sec"
                  << std::endl;
        ...
        auto scan_duration =
            std::chrono::duration_cast<std::chrono::microseconds>(scan_end -
                                                                  start);
    ...
    std::cout << "\nTKRZW TreeDBM provides sorted key-value storage with:"
              << std::endl;
    std::cout << "  - Keys stored in lexicographic order" << std::endl;
    std::cout << "  - Very efficient prefix scanning (no full iteration)"
              << std::endl;
    ...
    std::cout << "  - Better for scan-heavy workloads than HashDBM"
              << std::endl;
