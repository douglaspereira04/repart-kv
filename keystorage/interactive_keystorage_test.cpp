...
template <typename StorageType, typename ValueType> class KeyStorageWrapperImpl
    : public KeyStorageWrapper {
private:
...
void print_help() {
    ...
    std::cout << "  get(\"key\")                  - Get a value by key"
              << std::endl;
    std::cout << "  put(\"key\", value)           - Put a key-value pair"
              << std::endl;
    std::cout << "  scan(\"start_key\", limit)    - Scan entries >= start_key "
                 "(up to limit)"
              << std::endl;
...
int main() {
    ...
    std::cout << "  2. TkrzwHashKeyStorage (hash-based, unordered)"
              << std::endl;
    ...
    if (type_choice == 1) {
        ...
        switch (storage_choice) {
            case 1:
                storage = std::make_unique<
                    KeyStorageWrapperImpl<MapKeyStorage<int>, int>>(
                    "MapKeyStorage", "int");
                break;
            case 2:
                storage = std::make_unique<
                    KeyStorageWrapperImpl<TkrzwHashKeyStorage<int>, int>>(
                    "TkrzwHashKeyStorage", "int");
                break;
            case 3:
                storage = std::make_unique<
                    KeyStorageWrapperImpl<TkrzwTreeKeyStorage<int>, int>>(
        ...
        switch (storage_choice) {
            case 1:
                storage = std::make_unique<
                    KeyStorageWrapperImpl<MapKeyStorage<long>, long>>(
                    "MapKeyStorage", "long");
                break;
            case 2:
                storage = std::make_unique<
                    KeyStorageWrapperImpl<TkrzwHashKeyStorage<long>, long>>(
                    "TkrzwHashKeyStorage", "long");
                break;
            case 3:
                storage = std::make_unique<
                    KeyStorageWrapperImpl<TkrzwTreeKeyStorage<long>, long>>(
        ...
        switch (storage_choice) {
            case 1:
                storage = std::make_unique<
                    KeyStorageWrapperImpl<MapKeyStorage<uint64_t>, uint64_t>>(
                    "MapKeyStorage", "uint64_t");
                break;
            case 2:
                storage = std::make_unique<KeyStorageWrapperImpl<
                    TkrzwHashKeyStorage<uint64_t>, uint64_t>>(
                    "TkrzwHashKeyStorage", "uint64_t");
                break;
            case 3:
                storage = std::make_unique<KeyStorageWrapperImpl<
                    TkrzwTreeKeyStorage<uint64_t>, uint64_t>>(
    ...
    std::cout << "\nUsing: " << storage->get_name() << "<"
              << storage->get_type() << ">" << std::endl;
    print_help();
