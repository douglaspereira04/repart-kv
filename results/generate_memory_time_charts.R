...
#
# Usage:
#   Rscript generate_memory_time_charts.R [input_path] [output_path] [max_y] [max_x] [width] [height] [show_titles]
#
# Arguments:
...
chart_opts <- parse_chart_cli_args(
  ...
  default_output = "/home/douglas/Documentos/repart-kv/results/charts/memory_time",
...
for (csv_file in csv_files) {
  ...
  for (job in partition_chart_jobs(data)) {
    ...
    chart_data$mem_mib_mean <- chart_data$memory_kb_mean / 1024
    chart_data$mem_mib_min <- chart_data$memory_kb_min / 1024
    chart_data$mem_mib_max <- chart_data$memory_kb_max / 1024
    ...
    p <- ggplot(chart_data, aes(x = elapsed_s, y = mem_mib_mean, color = label, fill = label)) +
    ...
    output_file <- file.path(
      output_path,
      paste(c(workload, storage_engine, num_workers, thinking_time, sync_mode, p_parts, "memory_time.png"), collapse = ".")
    )
